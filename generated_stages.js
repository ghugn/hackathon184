(function () {
    ...
    for (let i = 0; i < total; i++) {
        ...
        const blueprint = {
            ...
            remixLevel: Number(Math.min(0.86, template.remixLevel + cycle * 0.03).toFixed(3)),
            gapScale: Number(Math.min(1.16, template.gapScale + Math.min(0.18, cycle * 0.03 + Math.floor(i / 5) * 0.002)).toFixed(3)),
            riseScale: Number(Math.min(1.08, template.riseScale + Math.min(0.12, cycle * 0.02 + Math.floor(i / 7) * 0.002)).toFixed(3)),
            widthScale: Number(Math.max(0.90, template.widthScale - Math.min(0.12, cycle * 0.02)).toFixed(3)),
            supportChance: Number(Math.max(0.18, template.supportChance - Math.min(0.12, cycle * 0.02)).toFixed(3)),
            connectorChance: Number(Math.min(0.28, template.connectorChance + Math.min(0.08, cycle * 0.015)).toFixed(3)),
            branchChance: Number(Math.min(0.18, template.branchChance + Math.min(0.08, cycle * 0.015)).toFixed(3)),
            bugBudgetBias: Math.min(2, template.bugBudgetBias + cycle),
            bugBudgetCap: stageIndex <= 4 ? 0 : stageIndex <= 8 ? Math.min(template.bugBudgetCap, 1) : template.bugBudgetCap,
            timeTrial: stageIndex >= 8 && stageIndex % 4 === 0 ? true : template.timeTrial,
            timeLimit: stageIndex >= 8 && stageIndex % 4 === 0 ? Math.max(22, (template.timeLimit || 32) - cycle) : (template.timeTrial ? Math.max(22, template.timeLimit - cycle) : 0),
        };
    ...
    window.AI_STAGE_LIBRARY = {
        ...
        model: 'deterministic-template-ring',
